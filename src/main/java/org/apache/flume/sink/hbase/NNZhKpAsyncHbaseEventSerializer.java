...
/**
 * @author zx it's for hbase table zx_test
 */
public class NNZhKpAsyncHbaseEventSerializer implements
		AsyncHbaseEventSerializer {
	private byte[] table;// hbase table zx_test
	private byte[] cf;// hbase table zx_test columnfamily info
	private byte[][] payload;
	private byte[][] payloadColumn;
	// hbase table zx_test columnfamily info columns
	private final String payloadColumnSplit = "#"; // columns#columns#columns
		...
		if (payloadColumn != null) {
			...
			try {
				...
				case CUSTOMHASH:
					...
					rowKey = NNZhRowKeyUtil.getZHLogsRowKey(rowSuffix,
							payloadvalue);
					System.out
				...
				System.out
						.println("this.payload.length.............................."+this.payload.length);
	...
	@Override
	public void configure(Context context) {// context --/flume/conf/*.conf
		...
		if (pCol != null && !pCol.isEmpty()) {
			...
			// userid,username,date,module,modular,ipadd,action,Collogs
			payloadColumn = new byte[pCols.length][];
			System.out.println("pCols.length" + pCols.length);
			for (int i = 0; i < pCols.length; i++) {
				System.out.println("pCols[i]" + pCols[i]);
				payloadColumn[i] = pCols[i].toLowerCase().getBytes(
						Charsets.UTF_8);
			}
			System.out.println("payloadColumn.length" + payloadColumn.length);
		}
	...
	public void setEvent(Event event) {
		System.out.println("进入一次新的event,the event's context ");
		...
		// long randomId;
		if (subBody.length > 1) {
			System.out.println("s" + subBody[4]);
			System.out.println("subBody[4]" + subBody[4]);
			if (subBody[4].equals("001008")) {
				...
				for (int i = 0; i < subBody.length; i++) {
					...
					System.out.println("payloadColumn[" + i + "]"
							+ payloadColumn[i]);
					if ((new String(this.payloadColumn[i])
							.equals(this.rowSuffixCol))) {
						System.out.println("subBody[" + i + "]" + subBody[i]);
						this.rowSuffix = subBody[i];
						System.out
								.println("come into the rowPrefixCol==payloadColumn");
						System.out
								.println("------------------------------------>>>>>>>>>>>>>rowSuffix:"
										+ subBody[i]);
					}
