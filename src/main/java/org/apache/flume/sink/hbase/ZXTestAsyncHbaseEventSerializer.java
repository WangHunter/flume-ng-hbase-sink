 ...
 */
public class ZXTestAsyncHbaseEventSerializer implements
		...
		if (payloadColumn != null) {
			...
			try {
				...
				case CUSTOMHASH:
					...
					rowKey = NNZhRowKeyUtil
							.getZXRowKey(rowSuffix, payloadvalue);
					System.out
	...
	public void setEvent(Event event) {
		...
		System.out.println("strBody----------------->" + strBody);
		System.out.println("subBody.length----------------->" + subBody.length);
		for (int i = 0; i < subBody.length; i++) {
			...
			if ((new String(this.payloadColumn[i]).equals(this.rowSuffixCol))) {
				...
				System.out.println("come into the rowPrefixCol==payloadColumn");
				System.out
						.println("------------------------------------>>>>>>>>>>>>>rowSuffix:"
								+ subBody[i]);
			}
